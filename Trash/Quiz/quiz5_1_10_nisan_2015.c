...
int main() {
	...
	printf("\nFark: %d:%02d:%02d -%d:%02d:%02d = %d:%02d:%02d\n\n",
			bitis.saat, bitis.dakika, bitis.saniye,
			baslama.saat, baslama.dakika, baslama.saniye,
			fark_deger.saat, fark_deger.dakika, fark_deger.saniye
			);
...
struct Zaman fark(struct Zaman bitis, struct Zaman baslama) {
	...
	int saniye_toplam = {
		(bitis.saat * 3600) - (baslama.saat * 3600)
		+ (bitis.dakika * 60) - (baslama.dakika * 60)
		+ (bitis.saniye) - (baslama.saniye)
	};

	fark_deger.saat = (saniye_toplam / 3600);
	fark_deger.dakika = ((saniye_toplam / 60) % 60);
	fark_deger.saniye = (saniye_toplam % 60);
