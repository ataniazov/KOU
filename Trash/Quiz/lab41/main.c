...
int soru_8(const char s[]) {
	...
	/*
		for(i=0; s[i] != '\0'; i++) {
			rakam_sayisi += isdigit(s[i]);
			harf_sayisi += isalpha(s[i]);
			bosluk_sayisi += isspace(s[i]);
		}
	*/
...
void soru_9(const char s[], char tersten[]) {
	/** TODO: s stringinin tersten yazilisini tersten isimli stringe kaydedin */
		...
		for(i=karakter_s-1, j=0; i >= 0; i--, j++) {
			tersten[j] = s[i];
		}
	tersten[j] = '\0';
